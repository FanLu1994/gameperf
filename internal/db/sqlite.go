...
func (d *DB) CreateSession(s *model.Session) error {
	_, err := d.conn.Exec(
		`INSERT INTO sessions (id, name, process, pid, status, start_time, interval, tags) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`,
		s.ID, s.Name, s.Process, s.PID, s.Status, s.StartTime, s.Interval, s.Tags,
	)
	return err
}

func (d *DB) ListSessions() ([]model.Session, error) {
	rows, err := d.conn.Query(`SELECT id, name, process, pid, status, start_time, end_time, interval, tags FROM sessions ORDER BY start_time DESC`)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	...
	for rows.Next() {
		...
		if err := rows.Scan(&s.ID, &s.Name, &s.Process, &s.PID, &s.Status, &s.StartTime, &endTime, &s.Interval, &s.Tags); err != nil {
			return nil, err
		}
		if endTime.Valid {
...
func (d *DB) GetSession(id string) (*model.Session, error) {
	...
	var endTime sql.NullTime
	err := d.conn.QueryRow(
		`SELECT id, name, process, pid, status, start_time, end_time, interval, tags FROM sessions WHERE id = ?`, id,
	).Scan(&s.ID, &s.Name, &s.Process, &s.PID, &s.Status, &s.StartTime, &endTime, &s.Interval, &s.Tags)
	if err != nil {
		return nil, err
	}
	if endTime.Valid {
...
func (d *DB) SaveSystemInfo(info *model.SystemInfo) error {
	_, err := d.conn.Exec(
		`INSERT OR REPLACE INTO system_info (session_id, os, arch, cpu_model, cpu_cores, total_memory, gpu_name, gpu_driver, gpu_vram) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
...
func (d *DB) GetSystemInfo(sessionID string) (*model.SystemInfo, error) {
	...
	err := d.conn.QueryRow(
		`SELECT session_id, os, arch, cpu_model, cpu_cores, total_memory, gpu_name, gpu_driver, gpu_vram FROM system_info WHERE session_id = ?`, sessionID,
...
func scanSample(rows interface{ Scan(...interface{}) error }) (*model.Sample, error) {
	var s model.Sample
	err := rows.Scan(
...
func (d *DB) GetSummary(sessionID string) (*model.SessionSummary, error) {
	...
	gpuMems := make([]float64, 0)
	gpuPowers := make([]float64, 0)
	fpss := make([]float64, 0)
	frameTimes := make([]float64, 0)
	janks := int32(0)
	diskReadBPS := make([]float64, 0)
	diskWriteBPS := make([]float64, 0)
	netSentBPS := make([]float64, 0)
	netRecvBPS := make([]float64, 0)
	...
	for i, s := range samples {
		...
		if s.GPUUtil > 0 {
			gpus = append(gpus, s.GPUUtil)
		}
		if s.GPUTemp > 0 {
			gpuTemps = append(gpuTemps, s.GPUTemp)
		}
		if s.GPUMem > 0 {
			gpuMems = append(gpuMems, s.GPUMem)
		}
		if s.GPUPower > 0 {
			gpuPowers = append(gpuPowers, s.GPUPower)
		}
		if s.FPS > 0 {
			fpss = append(fpss, s.FPS)
		}
		if s.FrameTime > 0 {
			frameTimes = append(frameTimes, s.FrameTime)
		}
		janks += s.JankCount

		if s.DiskReadBPS > 0 {
			diskReadBPS = append(diskReadBPS, s.DiskReadBPS)
		}
		if s.DiskWriteBPS > 0 {
			diskWriteBPS = append(diskWriteBPS, s.DiskWriteBPS)
		}
		if s.NetSentBPS > 0 {
			netSentBPS = append(netSentBPS, s.NetSentBPS)
		}
		if s.NetRecvBPS > 0 {
			netRecvBPS = append(netRecvBPS, s.NetRecvBPS)
		}

		if s.Threads > summary.MaxThreads {
			summary.MaxThreads = s.Threads
		}
		if s.HandleCount > summary.MaxHandleCount {
			summary.MaxHandleCount = s.HandleCount
		}
	}

	// CPU
	sort.Float64s(cpus)
	summary.AvgCPU = avg(cpus)
	summary.MinCPU = cpus[0]
	summary.MaxCPU = cpus[len(cpus)-1]
	summary.P95CPU = percentile(cpus, 95)
	summary.P99CPU = percentile(cpus, 99)

	// Memory
	sort.Float64s(mems)
	summary.AvgMemory = avg(mems)
	summary.MinMemory = mems[0]
	summary.MaxMemory = mems[len(mems)-1]
	summary.P95Memory = percentile(mems, 95)
	summary.P99Memory = percentile(mems, 99)

	// GPU
	if len(gpus) > 0 {
		sort.Float64s(gpus)
		summary.AvgGPU = avg(gpus)
		summary.MaxGPU = gpus[len(gpus)-1]
		summary.P95GPU = percentile(gpus, 95)
	}
	if len(gpuTemps) > 0 {
		sort.Float64s(gpuTemps)
		summary.MaxGPUTemp = gpuTemps[len(gpuTemps)-1]
	}
	if len(gpuPowers) > 0 {
		summary.AvgGPUPower = avg(gpuPowers)
	}
	if len(gpuMems) > 0 {
		sort.Float64s(gpuMems)
		summary.MaxGPUMem = gpuMems[len(gpuMems)-1]
	}
	...
	if len(fpss) > 0 {
		...
		summary.MaxFPS = fpss[len(fpss)-1]
		summary.P1FPS = percentile(fpss, 1)
		summary.P5FPS = percentile(fpss, 5)
		summary.P95FPS = percentile(fpss, 95)
		// FPS稳定性: 1 - (std/avg), 越接近1越稳定
		summary.FPSStability = 1.0 - (stddev(fpss)/avg(fpss))
		if summary.FPSStability < 0 {
			summary.FPSStability = 0
		}
	}
	...
	if len(frameTimes) > 0 {
		...
		summary.AvgFrameTime = avg(frameTimes)
		summary.P95FrameTime = percentile(frameTimes, 95)
		summary.P99FrameTime = percentile(frameTimes, 99)
		summary.MaxFrameTime = frameTimes[len(frameTimes)-1]
	}

	// Jank
	summary.TotalJankCount = janks
	stutterRates := make([]float64, 0)
	for _, s := range samples {
		if s.StutterRate > 0 {
			stutterRates = append(stutterRates, s.StutterRate)
		}
	}
	if len(stutterRates) > 0 {
		summary.AvgStutterRate = avg(stutterRates)
	}

	// Disk I/O
	if len(diskReadBPS) > 0 {
		summary.AvgDiskReadBPS = avg(diskReadBPS)
		sort.Float64s(diskReadBPS)
		summary.MaxDiskReadBPS = diskReadBPS[len(diskReadBPS)-1]
	}
	if len(diskWriteBPS) > 0 {
		summary.AvgDiskWriteBPS = avg(diskWriteBPS)
		sort.Float64s(diskWriteBPS)
		summary.MaxDiskWriteBPS = diskWriteBPS[len(diskWriteBPS)-1]
	}

	// Network I/O
	if len(netSentBPS) > 0 {
		summary.AvgNetSentBPS = avg(netSentBPS)
		sort.Float64s(netSentBPS)
		summary.MaxNetSentBPS = netSentBPS[len(netSentBPS)-1]
	}
	if len(netRecvBPS) > 0 {
		summary.AvgNetRecvBPS = avg(netRecvBPS)
		sort.Float64s(netRecvBPS)
		summary.MaxNetRecvBPS = netRecvBPS[len(netRecvBPS)-1]
	}
...
func (d *DB) GetFrameTimeAnalysis(sessionID string) (*model.FrameTimeAnalysis, error) {
	...
	analysis := &model.FrameTimeAnalysis{
		SessionID:  sessionID,
		FrameCount: 0,
	}
	...
	// 构建直方图 — 自动分桶
	minFT := math.Min(frameTimes[0], frameTimes[0])
	maxFT := frameTimes[0]
	...
	for i := 0; i < bucketCount; i++ {
		buckets[i] = model.Bucket{
			RangeStart: minFT + float64(i)*bucketSize,
			RangeEnd:   minFT + float64(i+1)*bucketSize,
			Count:      0,
		}
	}
	...
	// 检测卡顿帧 — 帧时间 > 前一帧 * 2 视为 Jank，> * 3 为 BigJank
	...
	for i := 1; i < len(frameTimes); i++ {
		...
		if prev > 0 && curr > prev*2 {
			...
			jankFrames = append(jankFrames, model.JankFrame{
				Elapsed:   elapsed,
				FrameTime: curr,
				Severity:  severity,
			})
			...
			if inStutter {
				...
				stutterSections = append(stutterSections, model.StutterSection{
					StartElapsed: stutterStart,
					EndElapsed:   elapsed,
					Duration:     elapsed - stutterStart,
					FrameCount:   stutterCount,
				})
				inStutter = false
			}
		}
